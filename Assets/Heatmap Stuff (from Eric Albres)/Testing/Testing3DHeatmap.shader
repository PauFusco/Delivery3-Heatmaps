...
Shader "Unlit/Testing3DHeatmap"
{
  Properties
  {
    ...
    _Color0("Color 0",Color) = (0,0,0,1)
      ...
      _Diameter("Diameter",Range(0,100)) = 20.0
    ...
    SubShader
    {
      ...
      Pass
      {
        ...
        fixed4 frag(v2f i) : SV_Target
        {
          ...
          float3 world_pos = mul(unity_ObjectToWorld, float4(i.vertex.xyz, 1)).xyz;
          ...
          for (float i = 0.0; i < _HitCount; i++)
          {
            ...
            totalWeight += 0.5 * distsq(world_pos, work_pt) * pt_intensity * _Strength * (1 + sin(_Time.y * _PulseSpeed));
          }
          return col + float4(getHeatForPixel(totalWeight), .5);

          return fixed4(world_pos, 0.5);
        }
